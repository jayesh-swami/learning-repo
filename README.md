# learning-repo
Repository to store all my notes, resources, paper reviews etc. As an ambition to best keep track of my learning, each directory contains a `metadata.yaml` file that serves as the single source of truth for the progress, context, and academic connections. The format is as follows - 
...
| `type` | `string` | Type of the resource (course/paper-review/others) |
...
| `last_revision_date` | `date` | The last time I did a revision of this course/paper (`YYYY-MM-DD`). |
...
| ↳ `program` | `string` \| `null` | The specific degree or program name (if applicable) the course/paper is associated with. |
| ↳ `credential_earned`| `boolean` | `true` if a certificate or degree was obtained. |
| ↳ `related_papers` | `array[string]`| List of academic papers read or planned to read during this course/papepr. |
